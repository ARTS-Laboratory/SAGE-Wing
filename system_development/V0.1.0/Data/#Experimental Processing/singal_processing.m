...
minimumPeakDistanceSeconds = 0.15; % Minimum allowed time between detected peaks/troughs
hysteresisHighThreshold = [];  % Empty uses robust automatic estimate
hysteresisLowThreshold = [];   % Empty uses robust automatic estimate
frequencySmoothingSeconds = 1.0; % Moving-mean smoothing duration for peak-to-peak frequency
saveFrequencyCsv = true;
...
options = { ...
    'Plot Frequency (on right y-axis)', ...
    'Generate Event Table CSV', ...
    'Save PNG Plot', ...
    'Save PDF Plot', ...
    'Save JPG Plot', ...
    'Save Frequency CSV', ...
...
[selected, ok] = listdlg( ...
    'ListString',options, ...
    'PromptString', 'Select features to enable:', ...
    'SelectionMode', 'multiple', ...
    'InitialValue', 1:numel(options), ... % All checked by default
    'ListSize', [250, 150], ...
    'Name', 'Feature Selector' ...
);

if ~ok
    selected = 1:numel(options); % Fallback to all if canceled
end

% Set logical flags based on selection
plotFrequency   = ismember(1, selected);
saveEventCsv    = ismember(2, selected);
savePng         = ismember(3, selected);
savePdf         = ismember(4, selected);
saveJpg         = ismember(5, selected);
saveFreqCsv     = ismember(6, selected);
plotFreqGraph   = ismember(7, selected);
...
medianDt = median(diff(timeValues), 'omitnan');
if isfinite(medianDt) && medianDt > 0
    minimumPeakDistance = max(1, round(minimumPeakDistanceSeconds / medianDt));
else
    warning('Invalid time spacing detected. Using sample-based minimum peak distance fallback.');
    minimumPeakDistance = max(1, round(0.02 * nSamples));
end
...
peakTimes = timeValues(peakLocations);
[frequencyTimes, peakToPeakFrequencyHz, smoothedPeakToPeakFrequencyHz, ...
    previousPeakTimes, currentPeakTimes, peakIntervals] = calculatePeakToPeakFrequency( ...
    peakTimes, frequencySmoothingSeconds);
...
frequencyPlotJpgPath = fullfile(inputFolder, [inputName '_peak_to_peak_frequency.jpg']);
frequencyCsvPath = fullfile(inputFolder, [inputName '_peak_to_peak_frequency.csv']);
...
frequencyTable = table( ...
    frequencyTimes(:), ...
    peakToPeakFrequencyHz(:), ...
    smoothedPeakToPeakFrequencyHz(:), ...
    previousPeakTimes(:), ...
    currentPeakTimes(:), ...
    peakIntervals(:), ...
    'VariableNames', {'Time', 'PeakToPeakFrequencyHz', 'SmoothedPeakToPeakFrequencyHz', 'PreviousPeakTime', 'CurrentPeakTime', 'PeakIntervalSeconds'});

if saveFrequencyCsv
    writetable(frequencyTable, frequencyCsvPath);
end
...
if ~isempty(frequencyTimes) && plotFrequency
    yyaxis right;
    plot(frequencyTimes, smoothedPeakToPeakFrequencyHz, '--', ...
        'Color', [0.85 0.70 0.10], ...
        'LineWidth', 1.2, ...
        'DisplayName', 'Smoothed peak-to-peak frequency');
    ylabel('Peak-to-Peak Frequency (Hz)');
end

xlim([min(timeValues), max(timeValues)]);
...
if savePng
    exportgraphics(figureHandle, plotPngPath, 'Resolution', 300, 'ContentType', 'image');
end
...
if plotFreqGraph
    frequencyFigureHandle = figure('Color', 'w', 'Name', 'Peak-to-Peak Frequency');
    hold on;
    if ~isempty(frequencyTimes)
        plot(frequencyTimes, peakToPeakFrequencyHz, '-', ...
            'Color', [0.45 0.45 0.45], ...
            'LineWidth', 1.0, ...
            'DisplayName', 'Raw peak-to-peak frequency');
        plot(frequencyTimes, smoothedPeakToPeakFrequencyHz, '--', ...
            'Color', [0.85 0.70 0.10], ...
            'LineWidth', 1.2, ...
            'DisplayName', 'Smoothed peak-to-peak frequency');
        ...
        text(0.5, 0.5, 'Fewer than 2 detected peaks', ...
            'Units', 'normalized', ...
    ...
    xlabel('Time');
    ylabel('Frequency (Hz)');
    title('Peak-to-Peak Frequency from Detected Peaks');
    hold off;
    
    try
        exportgraphics(frequencyFigureHandle, frequencyPlotJpgPath, 'Resolution', 300);
    catch
        saveas(frequencyFigureHandle, frequencyPlotJpgPath);
    end
end
...
if plotFreqGraph
    fprintf('Peak-to-peak frequency JPG saved to:\n%s\n', frequencyPlotJpgPath);
end
if saveFreqCsv
    fprintf('Peak-to-peak frequency CSV saved to:\n%s\n', frequencyCsvPath);
end
...
function [frequencyTimes, peakToPeakFrequencyHz, smoothedPeakToPeakFrequencyHz, previousPeakTimes, currentPeakTimes, peakIntervals] = calculatePeakToPeakFrequency(peakTimes, smoothingSeconds)
    if ~isscalar(smoothingSeconds) || ~isfinite(smoothingSeconds) || smoothingSeconds <= 0
        error('frequencySmoothingSeconds must be a positive finite number.');
    end
    ...
    if numel(peakTimes) < 2
        frequencyTimes = zeros(0, 1);
        peakToPeakFrequencyHz = zeros(0, 1);
        smoothedPeakToPeakFrequencyHz = zeros(0, 1);
        previousPeakTimes = zeros(0, 1);
        currentPeakTimes = zeros(0, 1);
        peakIntervals = zeros(0, 1);
        return;
    end

    previousPeakTimes = peakTimes(1:end - 1);
    currentPeakTimes = peakTimes(2:end);
    peakIntervals = diff(peakTimes);

    validIntervals = isfinite(peakIntervals) & peakIntervals > 0;
    previousPeakTimes = previousPeakTimes(validIntervals);
    currentPeakTimes = currentPeakTimes(validIntervals);
    peakIntervals = peakIntervals(validIntervals);

    frequencyTimes = currentPeakTimes;
    peakToPeakFrequencyHz = 1 ./ peakIntervals;
    ...
    if numel(frequencyTimes) >= 2
        medianFrequencyDt = median(diff(frequencyTimes), 'omitnan');
        if isfinite(medianFrequencyDt) && medianFrequencyDt > 0
            smoothingWindowPoints = max(1, round(smoothingSeconds / medianFrequencyDt));
        else
            smoothingWindowPoints = 1;
        end
    else
        smoothingWindowPoints = 1;
    end

    smoothedPeakToPeakFrequencyHz = movmean(peakToPeakFrequencyHz, smoothingWindowPoints, 'omitnan');
end
