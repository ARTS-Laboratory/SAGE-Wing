...
% Ask for an optional manual y-axis range (in Ohms) for the comparison plot
yRangeAnswer = inputdlg( ...
    {'Y-axis MINIMUM (in Ohms) for the comparison plot (leave blank for auto):', ...
     'Y-axis MAXIMUM (in Ohms) for the comparison plot (leave blank for auto):'}, ...
    'Optional Y-Axis Range', ...
    [1 90], ...
    {'', ''});

manualYLimOhms = [];
if ~isempty(yRangeAnswer)
    yMinStr = strtrim(yRangeAnswer{1});
    yMaxStr = strtrim(yRangeAnswer{2});

    if ~isempty(yMinStr) && ~isempty(yMaxStr)
        yMinOhms = str2double(yMinStr);
        yMaxOhms = str2double(yMaxStr);

        if ~isfinite(yMinOhms) || ~isfinite(yMaxOhms)
            errordlg('Y-axis min/max must be numeric values in Ohms.', 'Invalid Y-Axis Range');
            error('Y-axis min/max must be numeric values in Ohms.');
        end

        if yMaxOhms <= yMinOhms
            errordlg('Y-axis maximum must be greater than the minimum.', 'Invalid Y-Axis Range');
            error('Y-axis maximum (%.6g) must be greater than the minimum (%.6g).', yMaxOhms, yMinOhms);
        end

        manualYLimOhms = [yMinOhms, yMaxOhms];
    elseif xor(isempty(yMinStr), isempty(yMaxStr))
        errordlg('Please enter both a minimum and a maximum, or leave both blank for auto.', 'Invalid Y-Axis Range');
        error('Only one of the Y-axis min/max fields was filled in.');
    end
end
...
if ~isempty(manualYLimOhms)
    if maxResistance >= 1e6
        ylim(manualYLimOhms / 1e6);
    elseif maxResistance >= 1e3
        ylim(manualYLimOhms / 1e3);
    else
        ylim(manualYLimOhms);
    end
end
